...
def PortfolioStanDev(Weights,Stocks):
    '''
    args: Weights = list of floats Stocks = list of ticker
    returns: float
    '''
    ...
    T = 1
    for i in Stocks:
        stock = pd.read_csv("https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol={}&apikey=DNWR3TUKFOPP664I&datatype=csv".format(i.upper()))
        PercentChange = stock.close.pct_change().tolist()
        del PercentChange[0]
        StanDev =np.std(PercentChange)
        StandardDeviaton.append(round(StanDev,3))
        ClosePrices.append(stock.close.tolist())

        # Sleep is in place because the free API key takes 5 calls per minute
        # If the API is premium you can delete/comment the next block of code
    ...
    for i in range(len(Weights)):
        W = Weights[i]**2
        V = StandardDeviaton[i]**2
        WV = W *V
        WeightedVariance.append(WV)
    
    SumWeightedVariance = np.sum(WeightedVariance)
    
    comb = combinations(range(len(Stocks)),2)
    WeightedCovariance = []
    for i in comb:
        x = i[0]
        y = i[1]
        Cor = np.corrcoef([ClosePrices[i[0]],ClosePrices[i[1]]])
        W1 = Weights[x]
        W2 = Weights[y]
        Cov = Cor[0][1] * StandardDeviaton[i[0]] * StandardDeviaton[i[1]]
        WCV = 2 * W1 * W2 * Cov
        WeightedCovariance.append(WCV)   

    SumWeightedCovariance = np.sum(WeightedCovariance)
    
    PortVariance = SumWeightedCovariance+SumWeightedVariance
